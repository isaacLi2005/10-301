...
\newcommand{\outDate}{Friday, Mar 27th}
\newcommand{\dueDate}{Sunday, Apr 5th}
\newcommand{\taNames}{Harry, Opally, Sam, Zachary, Neural the Narwhal}
\newcommand{\homeworktype}{\string written, programming}
\newcommand{\exitPollLink}{\string https://docs.google.com/forms/d/e/1FAIpQLSfWndwOuvsx6FoL79RooeiPkGs50UPZK21rK64lrmgSjw9RDQ/viewform?usp=header}
...
\subpart Now you will derive the steps of the backpropagation algorithm that lead to the computation of $\frac{dJ}{d\Wv_{hh}}$. For all parts of this question, please write your answer in terms of $\Wv_{hh}$, $\Wv_{hy}$, $\yv$, $\hat{\yv}$, $\hv$, and any additional terms specified in the question (note: this does not mean that every term listed shows up in every answer, but rather that you should simplify terms into these as much as possible when you can).
    ...
    \begin{your_solution}[title=Work,height=5.5cm,width=14cm]
    % YOUR ANSWER
    \end{your_solution}
    ...
    \subsubpart[2] What is $g_{\Wv_{hh}} = \frac{\partial J}{\partial \Wv_{hh}}$? Write your solution in terms of $\gv_{\hv_i}$ and $\hv_i$ in the first box, and show your work in the second. \\
    Hint: $\gv_{\hv_i}$  already captures the unrolled backward/future gradients for timestep $i$, so you only need to consider the contribution of $\Wv_{hh}$ at each timestep locally
...
\textbf{Select one:} This is a code snippet from \href{https://www.cs.cmu.edu/~mgormley/courses/10601//slides/lecture19-autodiff-icl.pdf#page=507}{lecture 19 slide 68}. In the context of the \texttt{method apply\_fwd()} inside the \texttt{Module} class, what is the primary role of the \texttt{tape.push(self)} command that pushes the module onto the tape?
...
{[}Total expected runtime on Colab T4: 15 minutes; A100: 10 minutes{]}
...
\begin{subparts}
    ...
    {[}Total expected runtime on Colab T4: 20 minutes, A100; 15 minutes{]}
    
    \begin{your_solution}[height=7cm,width=16cm]
        % YOUR TABLE HERE
    \end{your_solution}
    
    \subpart[2] Report the total time taken for each of the batch sizes from the previous part (a) in the form of a table. The table contains four rows, one each for \texttt{batch\_size} $32$, $64$, $128$, and $256$. Report time taken with either a T4 or an A100.
    ...
    \subpart[2] \textbf{Fill in the blank:} Complete the following statement accurately by selecting the best option for each blank, based on your observations from parts (a) and (b).  
    \doublespacing
    \begin{quote}
        Increasing batch size leads to \_\_\_\_\_(i)\_\_\_\_\_ training  and \_\_\_\_\_(ii)\_\_\_\_\_ in performance.\\
    ...
    \subsubpart 
    \begin{checkboxes} 
    \choice faster \choice slower \choice no difference \end{checkboxes}
    \subsubpart
    \begin{checkboxes} 
    \choice an increase \choice a decrease \choice no difference \end{checkboxes}
    \end{subsubparts}
\end{subparts}
...
\begin{subparts}
    ...
    {[}Total expected runtime on Colab T4: 30 minutes; A100: 8 minutes{]}
...
You should not use \texttt{transformers} for anything other than loading in the tokenizer. In the handout, we have given a file called environment.yaml. Please run \begin{lstlisting}
conda env create -f environment.yml
    
\end{lstlisting}
This will create the hw7-env environment. You should be using this environment when working on this homework locally.
\subsection{Google Colab (ALSO IMPORTANT)}

We recommend using Google Colab to test your code and answer the empirical questions throughout this assignment. You may test your code with the locally but there are occasionally errors that show up locally but not on the autograder or Google Colab. The free compute credits offered by Google for running your code on a GPU are not infinite, and running out of credits will make finishing homework 7 much harder. \textbf{You are not expected to and definitely do not need to pay for extra compute.} Google Colab is also offering a free year of Colab Pro to students. This will give you access to more compute hours and better GPUs. You can find more info \href{https://colab.research.google.com/signup}{here}. If you do end up running out, you can wait until the next day, make a new email account, or even try running it on Kaggle. However, as long as you're not using the GPU to debug you shouldn't run into credit issues. \textbf{TLDR: Test on CPU on Google Colab and then answer questions with a GPU on Google Colab.}
...
\begin{figure}[htp]
    ...
    \caption{Computation graph of hybrid RNN with attention.}
...
\end{align*}
where $\phi$ is an activation function (ReLU in this assignment).
...
\subsubsection{Hybrid RNN Language Model}
...
\begin{tabbing}
\=\texttt{\$ \textbf{python3} rnn.\textbf{py} [train/generate] [args\dots]}\\
\end{tabbing}
...
Each command (train or generate) has a different set of arguments to be passed in.
The arguments for train are described below:
\begin{enumerate}
    ...
    \item \texttt{<--model\textunderscore path>} string path to the \texttt{.pt} file where model will be saved after training.
\end{enumerate}

The arguments for generate are described below:
\begin{enumerate}
    \item \texttt{<--model\textunderscore path>} string path to the \texttt{model.pt} file where model will be loaded from.
    \item \texttt{<--prefix>} Prefix string to be generated from
    \item \texttt{<--num\textunderscore tokens>} Number of tokens to generate (\textbf{hyper-parameter})
    \item \texttt{<--temperature>} Temperature of decoding (\textbf{hyper-parameter})
\end{enumerate}
Below is an example command to run

\begin{lstlisting}
python3 rnn.py train --train_data data/train_stories.json \
...
\begin{lstlisting}
python3 rnn.py train --train_data data/tiny_train_stories.json \
...
\textit{Note:} For this assignment, you have 10 submissions to Gradescope before the deadline, but only the submission that you choose (defaulted to the most recent submission) will be graded.
\end{document}