...
\newcommand{\outDate}{Wednesday, February 25}
\newcommand{\dueDate}{Sunday, March 15 at 11:59pm}
\newcommand{\taNames}{Soham, Muskan, Jerry, Claire, Neural}
\newcommand{\homeworktype}{\string written+prog}
\newcommand{\exitPollLink}{\string https://forms.gle/aDSwBcEm7tbUYVDRA}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[
        ...
        \node[state] (z) [right = \distHThree of a] {$\zv = \text{ReLU}(\av)$};
...
Consider the neural network with one hidden layer shown in Figure \ref{fig:oneHL}. The input layer consists of 6 features $\xv = [x_1,...,x_6]^T$, the hidden layer has 3  nodes $\zv = [z_1,...,z_3]^T$, and the output layer is a probability distribution $\hat{y} = [\hat{y_1}, \hat{y_2}, \hat{y_3}, \hat{y_4}]^T$ over 4 classes derived by the softmax (such that $y_i$ is the probability of label $i$).
...
$\alpha^*_{j,i}$ represents the weight going \textit{to} the node $z_j$ in the hidden layer \textit{from} the node $x_i$ in the input layer (e.g. $\alpha^*_{1,2}$ is the weight from $x_2$ to $z_1$), and $\betav^*$ is defined similarly. We will use a ReLU activation function for the hidden layer and a softmax for the output layer.
...
\begin{equation}
\label{eqn:a}
a_j= \alpha_{j, b} + \sum_{i=1}^6 \alpha^*_{j,i}\cdot x_i,\,\, \forall j \in \{1,\ldots,3\}
\end{equation}

Activation at the first (hidden) layer:
\begin{align}
\label{eqn:z}
z_j &= \text{ReLU}(a_j) = \max(0, a_j),\,\, \forall j \in \{1,\ldots,3\}
\end{align}
Equivalently, we can write this as vector operation where the ReLU activation is applied individually to each element of the vector $\av$:
\begin{align}
\label{eqn:zv}
\zv &= \text{ReLU}(\av)
\end{align}
...
\begin{equation}
\label{eqn:b}
b_k = \beta_{k, b} + \sum_{j=1}^3 \beta^*_{k,j}\cdot z_j,\,\, \forall k \in \{1,\ldots,4\}
\end{equation}

Activation at the second (output) layer:
\begin{equation}
\label{eqn:yhat}
\hat{y}_k = \frac{\exp(b_k)}{\sum\limits_{l=1}^4 \exp(b_l)},\,\, \forall k \in \{1,\ldots,4\}
\end{equation}
...
\begin{equation}
\label{eqn:loss}
  \ell(\hat{\yv},\yv) = - \sum_{i=1}^4 y_i \ln(\hat{y}_i)
\end{equation}

\paragraph{Prediction}
When doing prediction, we will predict the $\argmax$ of the output layer. For example, if $\hat{y}_1=0.3, \hat{y}_2=0.2, \hat{y}_3=0.4, \hat{y}_4=0.1$ we would predict class 3. If the true class from the training data was $2$ we would have a one-hot vector $\yv$ with values $y_1=0$, $y_2=1$, $y_3=0$, $y_4=0$.
...
$$\boldsymbol{\beta}=
    \begin{bmatrix}
    \textcolor{blue}{\beta_{1,0}} & \beta_{1,1} & \beta_{1,2} & \beta_{1,3} \\
    \textcolor{blue}{\beta_{2,0}} & \beta_{2,1} & \beta_{2,2} & \beta_{2,3} \\
    \textcolor{blue}{\beta_{3,0}} & \beta_{3,1} & \beta_{3,2} & \beta_{3,3} \\
    \textcolor{blue}{\beta_{4,0}} & \beta_{4,1} & \beta_{4,2} & \beta_{4,3}
    \end{bmatrix}$$
    ...
    Note that $\boldsymbol{\alpha}$ is the $\boldsymbol{\alpha^{*}}$ with the bias term folded in and $\xv^{(1)}$ is the $\xv^{*(1)}$ with the 1 folded in for the bias. Use them appropriately throughout the Question 1. 
    ...
    \begin{subparts}
        ...
        % YOUR ANSWER
        % Change \choice to \CorrectChoice for the appropriate selection/selections 
        \choice $\betav$ with just one row with non-zero values
        \choice $\betav$ with just one column with non-zero values
        \choice $\betav$ with all weights = 1
        \choice $\betav$ with all weights = 0
        \choice $\betav$ where: 1) $\betav_{ii}=1$ and 2) $\betav_{ij}=0$ where $i\neq j$
        \end{checkboxes}
...
\begin{subparts}
    \subpart[1] What is the derivative $\frac{\partial \ell}{\partial \hat{y}_i}$, where $1 \leq i \leq 4$? Your answer should be in terms of $y_i$ and $\hat{y}_i$. Recall that we define the loss $\ell(\hat{y}, y)$ as follows:
    \begin{align}
        \ell(\hat{\yv},\yv) = - \sum_{i=1}^4 y_i \ln(\hat{y}_i) \tag{\ref{eqn:loss}}
    \end{align}
    ...
    % YOUR ANSWER 
    \end{your_solution}
    \subpart[1] What is the derivative $\frac{\partial \ell}{\partial a_j}$ in terms of $\frac{\partial \ell}{\partial z_j}$ and $a_j$? \\

    \textbf{Hint 3:} Recall that if z = \text{ReLU(a)}, 
    \space $\frac{\partial z}{\partial a} =
    \begin{cases}
    1, & a > 0 \\
    0, & a \le 0
    \end{cases}$
    ...
    \subpart[1] What is the matrix $\frac{\partial \ell}{\partial \boldsymbol{\alpha}}$? Your answer should be in terms of $\frac{\partial \ell}{\partial a_j}$ and $\xv^{(1)}$.
...
\subpart[2] Train two single hidden layer neural networks, one using a \textbf{ReLU} activation function and another using a \textbf{sigmoid} activation function on the hidden layer. The single hidden layer in the two models should both have a dimension of \textbf{50}, and your learning rate should be \textbf{0.003}. Run the optimization for 100 epochs. 
...
\begin{your_solution}[title=Avg. Train + Validation Cross-Entropy Loss for ReLU and Sigmoid w/ 1 Hidden Layer,height=9cm,width=14.5cm]
% YOUR ANSWER 
\end{your_solution}

\subpart[2] Examine and comment on the difference in convergence between the two models. Which activation function allows the model to converge faster and why? 

\begin{your_solution}
[title=Answer,height=6cm,width=14.5cm]
\end{your_solution}

\newpage

\subpart[2] Now, try adding another hidden layer to both of the above neural networks. The hyperparameters for the models should be the same as above, and one neutral network should use \textbf{ReLU} and another should use \textbf{sigmoid} as their activation functions. Remember that adding another hidden layer means you should add both a linear layer and a ReLU/sigmoid layer in your code.
...
\subpart[2]  
Plot the graph using your chosen indices.

\begin{your_solution}[title=Plot of three misclassified data,height=8.5cm,width=14.5cm]
% YOUR ANSWER 
\end{your_solution}
...
To train, you should optimize this objective function using stochastic gradient descent (SGD), where the gradient of the parameters for each training example is computed via backpropagation. You should shuffle the training points when performing SGD using the provided \verb|shuffle| function, passing in the epoch number as a random seed. Note that with SGD, you are selecting a random term, $J^{(i)}$, from the sum in $J$:
\begin{equation}
\label{eq:sgd_celoss}
J^{(i)}(\alphav, \betav)= -\sum_{k=1}^{K} y_k^{(i)} \log (\hat{y}^{(i)}_k)
\end{equation}

You will use the (hopefully at this point) familiar SGD update rule to update the parameters of your model:
\begin{align}
\label{eq:sgd_update}
\alphav_{t+1} &\leftarrow \alphav_t - \gamma \frac{\partial J^{(i)}(\alphav_t, \betav_t)}{\partial \alphav_t} \\
\betav_{t+1} &\leftarrow \betav_t - \gamma \frac{\partial J^{(i)}(\alphav_t, \betav_t)}{\partial \betav_t}
\end{align}
...
Implementing a neural network can be tricky: the parameters are not just a simple vector, but a collection of many vectors and matrices; computational efficiency of the model itself becomes essential; the initialization strategy dramatically impacts overall learning quality;  other aspects which we will \emph{not} change (e.g. activation function, optimization method) also have a large effect. These tips should help you along the way:
...
\centering
\makebox[\textwidth][c]{%
  \includegraphics[width=1.4\textwidth]{hw5/img/F26_HW5_Debugging_Flowchart.pdf}
  }
\end{figure}
    % \newpage
