...
\newcommand{\outDate}{Monday, February 16}
\newcommand{\dueDate}{Wednesday, February 25 at 11:59 PM}
\newcommand{\taNames}{Changwook, Yuhan, Lovina, Sid, Markov, Neural}
\newcommand{\homeworktype}{\string written+prog}
\newcommand{\exitPollLink}{\string https://forms.gle/kQxbNGHSNWvPhQgSA}
...
\begin{parts}
    ...
    \begin{your_solution}[title=$\partial J(\thetav)/\partial \theta_1$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=$\partial J(\thetav)/\partial \theta_2$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=$\partial J(\thetav)/\partial \theta_3$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
    ...
    \begin{your_solution}[title=$\theta_1$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=$\theta_2$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
    \begin{your_solution}[title=$\theta_3$,height=1.8cm,width=5cm]
    % YOUR ANSWER 
    \end{your_solution}
...
\begin{parts}
\part[2] \sall Consider a matrix $\Xv \in \mathbb{R}^{N \times M}$ and matrix $\Vv \in \mathbb{R}^{M \times K}$. 
We can create a new matrix $\Uv \in \mathbb{R}^{N \times K}$ whose $(i, k)$-th element is the dot product between 
the $i$-th row of $\Xv$ and the $k$-th column of $\Vv$ using NumPy as follows:
\begin{lstlisting}[language=Python,escapechar=@]
# X and V are numpy ndarrays
# X.shape == (N, M), V.shape == (M, K)
