...
def feature_to_output_line(label: int, features: np.ndarray) -> str: 
    """
    Given a label and a feature vecotr (likely generated by turn_review_line_to_vector()) into a formatted output 
    line that rounds to 6 decimal points. 
    """
    parts = [f"{float(label):.6f}"] 
    parts += [f"{value:.6f}" for value in features]
